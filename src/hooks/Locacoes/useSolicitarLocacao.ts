...
import type {
    ResumoLocacaoCalculado,
    SolicitarLocacaoFormState,
} from '../../pages/Locacoes/SolicitarLocacao/SolicitarLocacao.types';
...
export function useSolicitarLocacao({ produto }: UseSolicitarLocacaoParams) {
    ...
    const setCampo = <K extends keyof SolicitarLocacaoFormState>(
        campo: K,
        valor: SolicitarLocacaoFormState[K],
    ) => {
        setForm((atual) => ({
            ...atual, [campo]: valor
        }));
    ...
    const resumo: ResumoLocacaoCalculado = useMemo(() => {
    ...
    // Monta os dados prontos para virar uma LocacaoData no contexto global de locacoes
    const montarDadosLocacao = () => ({
